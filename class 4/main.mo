...
shared(installer) actor class WalletManager(owners : [Principal], owners_number : Nat) = this {
    ...
    private var member_set = TrieSet.fromArray<Principal>(owners, Principal.hash, Principal.equal);
    ...
    private var restricted_canisters = TrieSet.fromArray<Principal>(owners, Principal.hash, Principal.equal);
    private stable var THE_M = do { 
        if (owners_number >= owners.size()) {
            owners.size()
        } else { 
            owners_number
        }
    ...
    public shared({caller}) func exeProposal(proposalId : Nat) : async Result.Result<() , Error> {
        ...
        switch(proposals.get(proposalId)){
            ...
            case (?v){
                if(TrieSet.size<Principal>(v.agreed)>=THE_M){
